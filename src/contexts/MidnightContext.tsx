...
export function MidnightProvider({ children }: { children: ReactNode }) {
  ...
  function friendlyError(err: any): string {
    ...
    if (msg.includes("'prove' returned an error") || msg.includes('Failed to fetch')) {
      return "Couldn't reach your local proof server. Some browsers block a public site like this one " +
        "from reaching your machine's localhost. Make sure the proof server is running " +
        '(docker run --rm -p 6300:6300 midnightntwrk/proof-server:8.1.0), or run this app locally ' +
        'with `npm run dev` for a more reliable connection. See docs/USAGE.md.';
    }
